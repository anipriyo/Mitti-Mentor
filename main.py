...
def predictSoil():
  if request.method == "POST":
    ...
    uploadedImg_path = os.path.join('static','uploads', secure_filename(nameUploadedImg))
    ...
    if (result == 0):
      return render_template("afterUploadSoil.html", soil_type='Alluvial', user_image_path=uploadedImg_path, desc=alluvial)
    elif (result == 1):
      return render_template("afterUploadSoil.html", soil_type='Black', user_image_path=uploadedImg_path, desc=black)
    elif result == 2:
      return render_template("afterUploadSoil.html", soil_type='Clay', user_image_path=uploadedImg_path, desc=clay)
    elif result == 3:
      return render_template("afterUploadSoil.html", soil_type='Red', user_image_path=uploadedImg_path, desc=red)
    else:
      return "An error occurred"
